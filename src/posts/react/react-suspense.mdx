...
React Suspense는 리액트 16.6 버전에서 실험적 기능으로 도입되었고, 18 버전부터 정식으로 지원되기 시작했습니다. 이 기능은 **비동기 작업이 완료되기 전에 컴포넌트의 렌더링을 일시 중단**하고, 주로 데이터를 가져오거나, **컴포넌트를 동적으로 로딩**할 때 사용되며, 컴포넌트가 **로딩 중일 때 로딩 스피너와 같은 Fallback UI를 쉽게 제공**하여 로딩 상태를 더 우아하게 처리할 수 있게 해줍니다.
...
1. 비동기 작업 감지: Suspense는 자식 컴포넌트에서 발생하는 비동기 작업(예: 데이터 fetching)을 감지합니다.
2. 렌더링 중단: 비동기 작업이 완료되지 않았을 경우, Suspense는 해당 컴포넌트의 렌더링을 일시 중단합니다.
3. Fallback UI 표시: 렌더링이 중단된 동안 Suspense는 지정된 fallback UI(예: 로딩 인디케이터)를 표시합니다.
4. 비동기 작업 완료 대기: Suspense는 모든 비동기 작업이 완료될 때까지 기다립니다.
5. 동시 렌더링: 모든 비동기 작업이 완료되면, Suspense는 중단되었던 컴포넌트들을 동시에 렌더링합니다.
6. 일관된 UI 업데이트: 이를 통해 부분적인 로딩 상태를 방지하고, 모든 데이터가 준비된 후 한 번에 UI를 업데이트할 수 있습니다.
